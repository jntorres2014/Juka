...
class IntelligentResponses(private val fishDatabase: FishDatabase) {
    ...
    fun getResponse(userInput: String): String {
        ...
        val mentionedSpecies = fishDatabase.fishSpeciesDB.keys.find { lowerInput.contains(it) }
        ...
        return when {
            ...
            mentionedSpecies != null -> {
                val fish = fishDatabase.fishSpeciesDB[mentionedSpecies]!!
            ...
            "exitosa" -> {
                val speciesAdvice = if (analysis.speciesFound.isNotEmpty()) {
                    ...
                    mainSpecies?.let { species ->
                        val fishInfo = fishDatabase.findFishByKeyword(species)
                        fishInfo?.let {
